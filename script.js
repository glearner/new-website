...
const feedURL = "https://medium.com/feed/@gkverma1094"
...
const parseMediumXML = xmlString => {
  parseString(xmlString, function (err, result) {
    ...
    for (let index = 0; index < item.length; index++) {
      ...
      console.log(`${__dirname}/${blogPath}/${fileName}.json`)
      fs.writeFile(
        `${__dirname}/${blogPath}/${fileName}.json`,
        JSON.stringify(item[index], null, 4),
        () => {
          console.log("Done")
        }
      )
